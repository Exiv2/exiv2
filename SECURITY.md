...
To qualify as a security issue, the bug **must** be reproducible on an official release of Exiv2, using the `exiv2` command-line application. Bugs that can *only* be triggered via one of the demo application from the `samples` sub-directory or via our fuzzing target in `fuzz` sub-directory are **not** security issues and can be reported by creating a (publicly visible) [issue](https://github.com/Exiv2/exiv2/issues). If you believe that the bug *is* a security issue, then please find a way to reproduce it using `exiv2` command-line application.
