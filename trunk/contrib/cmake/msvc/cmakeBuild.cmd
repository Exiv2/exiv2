...
if DEFINED _WEBREADY_ (
	...
	if DEFINED CURL_CMAKE (
	    ...
	    if NOT EXIST "%_ONCPATH_%"\curl (
            ...
            7z x %_CURL_%.tar
            cd "%_CURL_%\winbuild"
	        call:run nmake /f Makefile.vc mode=%_MODE_% vc=%_VC_% machine=%RawPlatform% "WITH_DEVEL=%_INSTALL_%" WITH_ZLIB=%_MODE_% ENABLE_WINSSL=YES && rem WITH_SSL=%_MODE_%
	        cd ..
	        copy/y       builds\libcurl-vc%_VC_%-%RawPlatform%-release-%_MODE_%-zlib-%_MODE_%-ipv6-sspi\lib\*        "%_LIBPATH_%"  
	        copy/y       builds\libcurl-vc%_VC_%-%RawPlatform%-release-%_MODE_%-zlib-%_MODE_%-ipv6-sspi\bin\*        "%_BINPATH_%"  
	        xcopy/yesihq builds\libcurl-vc%_VC_%-%RawPlatform%-release-%_MODE_%-zlib-%_MODE_%-ipv6-sspi\include\curl "%_ONCPATH_%"\curl
	        popd
	    )
	)
)
