...
// type definitions
     ...
     */
    using FindDecoderFct = DecoderFct (*)(const std::string &, uint32_t, IfdId);
    /*!
      @brief Type for a function pointer for a function to encode a TIFF component.
     */
    using FindEncoderFct = EncoderFct (*)(const std::string &, uint32_t, IfdId);
    /*!
     ...
     */
    using NewTiffCompFct = std::unique_ptr<TiffComponent> (*)(uint16_t, IfdId);
