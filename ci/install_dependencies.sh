...
case "$distro_id" in
    'fedora')
        dnf -y --refresh install gcc-c++ clang ninja-build cmake make expat-devel zlib-devel brotli-devel libssh-devel libcurl-devel gmock-devel which dos2unix glibc-langpack-en diffutils inih-devel
        ;;

    'debian')
        apt-get update
        apt-get install -y cmake  ninja-build g++ clang make libexpat1-dev zlib1g-dev libbrotli-dev libssh-dev libcurl4-openssl-dev libgmock-dev libxml2-utils libinih-dev
        # debian_build_gtest
        ;;
    ...
    'opensuse-tumbleweed')
        ...
        zypper --non-interactive install gcc-c++ clang ninja cmake make libexpat-devel zlib-devel libbrotli-devel libssh-devel libcurl-devel gmock which dos2unix libxml2-tools libinih-devel
        ;;
    *)
