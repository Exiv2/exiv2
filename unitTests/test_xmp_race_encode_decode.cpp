...
TEST(XmpRace, ConcurrentInitialization) {
  constexpr auto NUM_THREADS = 10;
  auto init_count = std::atomic<int>{0};

  // First terminate to reset state
  ...
  auto init_worker = [&]() {
    ...
    // Try to use the initialized parser
    try {
      auto ns = std::string("http://concurrent.init.test/");
      Exiv2::XmpProperties::registerNs(ns, "cinit");
    } catch (...) {
    }
  };
  ...
  for (int i = 0; i < NUM_THREADS; ++i) {
    threads.emplace_back(init_worker);
  }
...
// terminate() can be called while encode/decode is active

TEST(XmpRace, TerminateWhileActive) {
  ...
  auto encode_worker = [&]() {
    for (int i = 0; i < ITERATIONS && !stop; ++i) {
      try {
        ...
        Exiv2::XmpData xmpData;
        xmpData["Xmp.dc.format"] = "image/jpeg";
        xmpData["Xmp.dc.title"] = "Test " + std::to_string(i);
        std::string packet;
        if (Exiv2::XmpParser::encode(packet, xmpData) == 0) {
          success_count++;
        }
        ...
        // SDK might be terminated underneath us
      }
    }
  };

  auto lifecycle_worker = [&]() {
    for (int i = 0; i < ITERATIONS / 5 && !stop; ++i) {
      try {
        // Repeatedly terminate and re-initialize while encode is running
        Exiv2::XmpParser::terminate();
  ...
  auto t1 = std::thread(encode_worker);
  auto t2 = std::thread(lifecycle_worker);
  ...
  // If we survived, test passes (but Valgrind should catch races)
