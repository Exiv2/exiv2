...
struct EXIV2API XmpNsInfo {
  ...
  struct Prefix {
    ...
    std::string prefix_;  //!< The prefix string.
  };

  //! For comparison with namespace
  struct Ns {
    explicit Ns(std::string ns);
    std::string ns_;  //!< The namespace string
  };
...
class EXIV2API XmpKey : public Key {
 public:
  ...
  /// @param key The key string (i.e Xmp.Property.Key).
  /// @throw Error if the first part of the key is not 'Xmp' or the second part of the key cannot be parsed and
  /// converted to a known (i.e., built-in or registered) schema prefix.
  explicit XmpKey(const std::string& key);

  /// @brief Constructor to create an XMP key from a schema prefix and a property name.
  ...
  */
  ...
  std::unique_ptr<XmpKey> clone() const;
...
class EXIV2API XmpProperties {
 ...
 public:
  ...
  /// @return The title (label) of the property, 0 if the key is of an unknown property.
  static const char* propertyTitle(const XmpKey& key);

  /// @brief Return the description of the property.
  /// @param key The property key
  /// @return The description of the property, 0 if the key is of an unknown property.
  static const char* propertyDesc(const XmpKey& key);

  /// @brief Return the type for property \em key. The default for unknown keys is xmpText.
  /// @param key The property key
  ...
  ///
  /// If the key is a path to a nested property (one which contains a slash, like \c Xmp.MP.RegionInfo/MPRI:Regions),
  /// determines the innermost element (\c Xmp.MPRI.Regions) and returns its property information.
  /// @param key The property key
  /// @return A pointer to the property information, 0 if the key is of an unknown property.
  static const XmpPropertyInfo* propertyInfo(const XmpKey& key);

  /// @brief Return the namespace name for the schema associated with \em prefix.
  /// @param prefix Prefix
  /// @return The namespace name
  /// @throw Error if no namespace is registered with \em prefix.
  static std::string ns(const std::string& prefix);

  /// @brief Return the namespace description for the schema associated with \em prefix.
  /// @param prefix Prefix
  ...
  /// @param prefix Prefix
  /// @return Pointer to the built-in properties for prefix, may be 0 if none is configured in the namespace info.
  /// @throw Error if no namespace is registered with \em prefix.
  static const XmpPropertyInfo* propertyList(const std::string& prefix);

  /// @brief Return information about a schema namespace for \em prefix. Always returns a valid pointer.
  ...
  /// @return The prefix or an empty string if namespace \em ns is not registered.
  ...
  ///
  /// The function only has an effect if there is a namespace \em ns registered earlier, it does not unregister
  /// built-in namespaces.
